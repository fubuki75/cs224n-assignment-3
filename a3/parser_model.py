...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        @param embeddings (Tensor): word embeddings (num_words, embedding_size) 词数*嵌入维度数
        @param n_features (int): number of input features 输入特征数
        ...
        #nn.Embedding一般用于存储word embeddings，然后通过索引来调取对应单词的嵌入
        #nn.Embedding对象的输入为：(*)怎么样都行 下面的网址结合RNN推荐用[seq_len,batch_size](一般由[batch_size,seq_len]转换而来)，
        ##                        每个单词用index代替，并加上了结束符号EOS，进行了填补（每个样本等长），格式为LongTensor
        ##                 输出为：(*,embedding_size) 如果和上文一致，则为[seq_len,batch_size,embedding_size]
        ...
        ###         with the `nn.init.xavier_uniform_` function with `gain = 1` (default) 建立linear层，初始化
        ###     2) Construct `self.dropout` layer.建立dropout层
    ...
    def embedding_lookup(self, t):
        ...
        #pretrain_embeddings是shape为（token的数量，50：词向量维度数）的np.array,第i行是tok2id中id==i的token对应的嵌入
        #pretrain_embeddings(t)返回的标记对应的词向量，(batch_size,n_features,embed_size)
        #view函数将上面的格式转化为(batch_size,n_features*embed_size)，便于输入
    ...
    def forward(self, t):
        ...
        logits = self.hidden_to_logits(self.dropout(F.relu(self.embed_to_hidden(emb))))
